...
| Наименование атрибута | Описание | Название поля на UI | Тип | Уникальность | Обязательность | Валидация |
| --- | --- | --- | --- | --- | --- | --- |
| `id` | Внутренний суррогатный ключ | - | `UUID` | Да, первичный ключ | Да | Генерируется системой; должен соответствовать формату UUID |
| `name` | Уникальное название системы-источника | Наименование | `VARCHAR(255)` | Да | Да | Непустая строка, длина до 255 символов |
| `description` | Произвольное описание системы | Описание | `TEXT` | Нет | Нет | Допускается пустое значение |
| `isActiveSync` | Активна ли регулярная синхронизация | Включена | `BOOLEAN` | Нет | Да | Допустимые значения: `true`, `false`; значение по умолчанию - `false` |
| `syncStatus` | Текущий статус синхронизации | Статус | `VARCHAR(20)` | Нет | Да | Одно из значений: `IDLE`, `IN_PROGRESS`, `SUCCESS`, `ERROR`; значение по умолчанию - `IDLE` |
| `syncDate` | Время завершения последней синхронизации | Последняя синхронизация | `TIMESTAMPTZ` | Нет | Нет | Дата и время в формате ISO 8601; заполняется после завершения синхронизации |
| `createdAt` | Дата и время создания записи | Создан | `TIMESTAMPTZ` | Нет | Да | Заполняется системой при создании записи |
| `updatedAt` | Дата и время последнего изменения | Обновлен | `TIMESTAMPTZ` | Нет | Да | Заполняется системой при изменении записи; не может быть раньше `createdAt` |
...
| Наименование атрибута | Описание | Название поля на UI | Тип | Уникальность | Обязательность | Валидация |
| --- | --- | --- | --- | --- | --- | --- |
| `id` | Идентификатор записи лога | ID | `BIGINT` | Да, первичный ключ | Да | Генерируется системой |
| `externalSystemId` | Ссылка на систему-источник | Система-источник | `UUID` | Нет | Да | Должен ссылаться на существующую запись `external_system.id` |
| `status` | Статус сессии синхронизации | Статус | `VARCHAR(20)` | Нет | Да | Одно из значений: `IN_PROGRESS`, `SUCCESS`, `ERROR` |
| `syncStartedAt` | Дата и время начала синхронизации | Начало синхронизации | `TIMESTAMPTZ` | Нет | Да | Дата и время в формате ISO 8601 |
| `syncFinishedAt` | Дата и время завершения синхронизации | Конец синхронизации | `TIMESTAMPTZ` | Нет | Нет | Дата и время в формате ISO 8601; не может быть раньше `syncStartedAt` |
| `totalReceived` | Количество устройств, полученных из внешней системы | Получено устройств | `INT` | Нет | Да | Целое число больше или равно `0`; значение по умолчанию - `0` |
| `created` | Количество созданных камер | Создано камер | `INT` | Нет | Да | Целое число больше или равно `0`; значение по умолчанию - `0` |
| `updated` | Количество обновленных камер | Обновлено камер | `INT` | Нет | Да | Целое число больше или равно `0`; значение по умолчанию - `0` |
| `errors` | Количество ошибок обработки | Количество ошибок | `INT` | Нет | Да | Целое число больше или равно `0`; значение по умолчанию - `0` |
| `errorDetails` | Детали ошибок и расхождений | Детали ошибок | `JSONB` | Нет | Нет | Валидный JSON; допускается пустое значение |

#### 4.3. Расширение таблицы камер

| Наименование атрибута | Описание | Название поля на UI | Тип | Уникальность | Обязательность | Валидация |
| --- | --- | --- | --- | --- | --- | --- |
| `externalId` | Идентификатор камеры во внешней системе | External ID | `VARCHAR(255)` | Да, в паре с `externalSystemId` | Нет | Непустая строка длиной до 255 символов, если значение заполнено |
| `externalSystemId` | Система-источник камеры | Система-источник | `UUID` | Да, в паре с `externalId` | Нет | Должен ссылаться на существующую запись `external_system.id`; обязателен, если заполнен `externalId` |
| `fieldSources` | Метаданные источников значений по полям камеры | Источники полей | `JSONB` | Нет | Нет | Валидный JSON-объект со списком полей и источников значений |
